...
public function getUserShopCart($userid):array {
        ...
        WHERE s.productid = p.id and s.userid=:userid')->setParameter('userid',$userid);
    ...
    public function getUserShopCartCount($userid):Integer
    {
        $em = $this->getEntityManager();
        $query = $em ->createQuery('
        SELECT count(s.id) as shopcount FROM App\Entity\Shopcart s WHERE s.userid=:userid')->setParameter('userid',$userid);
